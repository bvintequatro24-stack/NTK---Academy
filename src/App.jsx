    ...
    <div style={{ animation:"slideIn .4s ease" }}>
      ...
      {loading ? (
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {ranking.map((p, i) => {
            const rawName = (p.email || "").split("@")[0] || "Usuário";
            const name = rawName
              .replace(/[._]+/g, " ")
              .trim()
  ...
  return (
    <div style={{ animation:"slideIn .4s ease" }}>
      <div style={{ display:"flex", alignItems:"center", gap:12, padding:"20px 0 16px" }}>
        <button onClick={onBack} style={{ background:"none", border:"none", color:"#868e96", cursor:"pointer", fontSize:22 }}>←</button>
        <h2 style={{ fontSize:19, fontWeight:900, margin:0, flex:1 }}>📊 Estatísticas por Pergunta</h2>
        <button onClick={loadStats} style={{ background:"rgba(255,255,255,.08)", border:"1px solid rgba(255,255,255,.15)", borderRadius:10, padding:"6px 10px", color:"#dee2e6", cursor:"pointer", fontSize:12 }}>↻</button>
      </div>
